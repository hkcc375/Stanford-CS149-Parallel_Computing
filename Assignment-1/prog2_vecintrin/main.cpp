...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  //
  __cs149_mask zeroMask, notZeroMask, activeMask, clampMask;
  __cs149_vec_int int_zeros = _cs149_vset_int(0), int_ones = _cs149_vset_int(1), count;
  __cs149_vec_float clamped = _cs149_vset_float(9.999999f), result, x;

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    
    // Handle case when N is not a multiple of VECTOR_WIDTH
    int remaining = min(VECTOR_WIDTH, N - i);
    __cs149_mask maskAll = _cs149_init_ones(remaining);

    // Load
    _cs149_vload_float(x, values + i, maskAll);
    _cs149_vload_int(count, exponents + i, maskAll);

    // result = 1.0 where exponent == 0
    _cs149_veq_int(zeroMask, count, int_zeros, maskAll);
    _cs149_vset_float(result, 1.f, zeroMask);

    // result = x where exponent != 0
    notZeroMask = _cs149_mask_not(zeroMask);
    notZeroMask = _cs149_mask_and(notZeroMask, maskAll);
    _cs149_vmove_float(result, x, notZeroMask);

    // count = count - 1
    _cs149_vsub_int(count, count, int_ones, maskAll);

    // While any lane still has count > 0
    _cs149_vgt_int(activeMask, count, int_zeros, maskAll);
    while (_cs149_cntbits(activeMask) > 0) {
        _cs149_vmult_float(result, result, x, activeMask);
        _cs149_vsub_int(count, count, int_ones, activeMask);
        _cs149_vgt_int(activeMask, count, int_zeros, maskAll);
    }

    // Clamp
    _cs149_vgt_float(clampMask, result, clamped, maskAll);
    _cs149_vmove_float(result, clamped, clampMask);
...
float arraySumVector(float* values, int N) {
  ...
  // All the lanes are active
  __cs149_mask maskAll = _cs149_init_ones();
  __cs149_vec_float temp;

  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    _cs149_vload_float(temp, values+i, maskAll); // result = values[i];
    _cs149_vadd_float(vecSum, vecSum, temp, maskAll); // vecSum += result;
  }

  float res[VECTOR_WIDTH];
  // Perform hadd on vecSum until we get the final sum in all lanes and then return the first element of res;
  for(int i = VECTOR_WIDTH; i > 1; i >>=1){
    _cs149_hadd_float(vecSum, vecSum);
    _cs149_interleave_float(vecSum, vecSum);
  }
  _cs149_vstore_float(res, vecSum, maskAll);
  return res[0];
}
