...
extern void sqrtThreaded(int N, float initialGuess, float* values, float* output);
...
int main() {
    ...
    for (unsigned int i=0; i<N; i++)
    {
        ...
        // Worst case for SIMD: 1 slow element per group of 8, rest are fast
        if (i % 8 == 0)
            values[i] = 2.999f;  // slow: many iterations (far from guess)
        else
            values[i] = 1.0f;   // fast: 0 iterations (guess is already correct)
    }
    ...
    for (int i = 0; i < 3; ++i) {
        double startTime = CycleTimer::currentSeconds();
        sqrtThreaded(N, initialGuess, values, output);
        double endTime = CycleTimer::currentSeconds();
